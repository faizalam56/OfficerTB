...
public class TableMainLayout extends RelativeLayout {
	...
	int  HeaderIndex1[] = {1, 2, 3, 4, 5, 6, 7, 8};
	String headerName[];
	String tableValues[][];
	int noOfRows;
	...
	public TableMainLayout(Context context, AttributeSet attrs) {
		...
		System.out.println("retrieveDataFromTable DOUBLE ");
	    }
...
//		this.resizeBodyTableRowHeight();
//
//		System.out.println("retrieveDataFromTable SINGLE " );
//	}
...
//	headerName = new String[8];

	noOfRows = MainActivity.sputumInfos.size();
	tableValues = new String[noOfRows][8];



		for (int i = 0; i < noOfRows; i++) {

			SputumInfo infos = MainActivity.sputumInfos.get(i);
			System.out.println("retrieveDataFromTable "+infos.getExaminDate());
			System.out.println("retrieveDataFromTable "+infos.getResult());
			tableValues[i][0] = infos.getSputumCounter();
			tableValues[i][1] = infos.getExaminDate();
			tableValues[i][2] = infos.getSputumSpecimen();
			tableValues[i][3] = infos.getVisualAppearance();
			tableValues[i][4] = infos.getResult();
			tableValues[i][5] = infos.getGrading();
			tableValues[i][6] = infos.getScanty();
			tableValues[i][7] = infos.getSubmitStatus();
...
//					System.out.println(tableValues[j][colm]+ " ---- "+ HeaderIndex1[colm]);
//       			}
//       			c.moveToNext();
//       		}
//           }
...
//	dbAdapter.close();
	...
	// generate table row of table A //**asif A  first left corner
	TableRow componentATableRow(){
		...
		textView.setWidth(100);//getResources().getDisplayMetrics().widthPixels/headers.length);
		componentATableRow.addView(textView);
		
		return componentATableRow;
	}
	
	// generate table row of table B //**asif B  Header of Columns
	TableRow componentBTableRow(){
		...
		for(int x=0; x<(headerFieldCount-1); x++){
			...
			textView.setTextColor(Color.WHITE);
			textView.setTypeface(null, Typeface.BOLD);
			textView.setWidth(200);//getResources().getDisplayMetrics().widthPixels/headers.length);
...
//
//                    //Toast.makeText(context,"table Row D",Toast.LENGTH_SHORT).show();
//
...
//
//                    Cursor c = dbAdapter.selectData(MyDatabaseAdapter.DATABASE_TABLE_1, null, "ROWID = "+(v.getId()), null, null, null);
...
//                        //	if  (c.moveToFirst()) {
//                        c.moveToFirst();
////                        int st1 = c.getColumnIndex("relation");
////                        String st21 = c.getString(2);
////                        String st = c.getString(st1);
//
...
//			}
			...
			if(x==0){
				...
				this.headerCellsWidth[x] = 100; //getResources().getDisplayMetrics().widthPixels/headers.length;
			//	this.headerCellsWidth[x] = this.viewWidth(((TableRow)this.tableA.getChildAt(0)).getChildAt(x));
			}else{
				this.headerCellsWidth[x] = 200; //getResources().getDisplayMetrics().widthPixels/headers.length;
			}
